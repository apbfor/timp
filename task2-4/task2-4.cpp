...
void mergeFiles(char *output_file, int n, int k)
{
    ...
    // отрезокаем для всех входных файлов, пока не достигнем конца
    while (count != i)
    {
...
// Используя алгоритм сортировки слиянием, создаем начальный отрезок и делим его на все среди выходных файлов
void createInitialRuns(char *input_file, int run_size, int num_ways) {
    ...
    // Выделяем динамическйи массив достаточного размера, чтобы вмещать отрезок размера run_size
    ...
    while (more_input)
    {
        // записываем run_size элементы в массив из входного файла
...
void externalSort(char* input_file,  char *output_file, int num_ways, int run_size) {
    // читаем входной файл, создаем начальный отрезок и назначаем для отрезка на выходных файлах
